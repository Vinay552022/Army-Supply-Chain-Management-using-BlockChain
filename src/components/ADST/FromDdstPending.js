...
import { TableDDSTtoADST } from '../TableDDSTtoADST';

function FromDdstPending({searchQuery}) {
    ...
    useEffect(()=>{
        ...
        const getPending=async ()=>{
            ...
            try{
                 ...
                 let requestIdsFromDDST=await contract.methods.getSentToUnits(DDSTmail).call();
  ...
  return (
    <>  
       ...
       <h2 className="mt-5" >{DDSTMail} Pending Requests</h2>
       </div>
       {console.log(filterMailIds,filterIds,filterQuantities,filterItems,"filter")}
        {
            ...
            return(
                <>   
                <TableDDSTtoADST Items={data} Quantities={Quantities[index]} Status={Status[index]} Ids={Ids[index]} ddstMail={DDSTMail} mailid={MailId} Timestamps={Timestamps[index]} mailIds={mailIds[index]} contract={contextUser.contract} account={contextUser.currentAccount} setFlag={setFlag}/>
                </>
...
export default FromDdstPending